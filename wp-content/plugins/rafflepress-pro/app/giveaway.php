 ...
 * Get Giveaway Lists
 */
function rafflepress_pro_get_giveaway_list() {
	if ( check_ajax_referer( 'rafflepress_pro_get_giveaway_list' ) ) {
		global $wpdb;
 ...
 * New Giveaway
 */
function rafflepress_pro_new_giveaway() {
	if ( isset( $_GET['page'] ) && $_GET['page'] == 'rafflepress_pro_builder' && isset( $_GET['id'] ) && $_GET['id'] == '0' ) {
		global $wpdb;
		$tablename = $wpdb->prefix . 'rafflepress_giveaways';
		...
		require_once RAFFLEPRESS_PRO_PLUGIN_PATH . 'resources/giveaway-templates/basic-giveaway.php';
		...
		$r = $wpdb->insert(
		 ...
		 $id = $wpdb->insert_id;
		if ( is_numeric( $id ) ) {
			$giveaway_name = esc_html__( 'New Giveaway', 'rafflepress-pro' ) . " (ID #$id)";
		} else {
			$giveaway_name = esc_html__( 'New Giveaway', 'rafflepress-pro' );
		}
		...
		$wpdb->update(
		 ...
		 wp_redirect( 'admin.php?page=rafflepress_pro_builder&id=' . $id . '#/template/' . $id );
		 exit();
	}
}
 ...
 * Giveaway Datatable
 */
function rafflepress_pro_giveaway_datatable() {
	if ( check_ajax_referer( 'rafflepress_pro_giveaway_datatable' ) ) {
		$data         = array( '' );
		$current_page = 1;
		...
		foreach ( $results as $v ) {
			...
			if ( empty( $v->active ) ) {
				$status = __( 'Disabled', 'rafflepress-pro' );
			}
		...
		$totalitems = rafflepress_pro_giveaway_get_data_total( $filter );
		$views      = rafflepress_pro_giveaway_get_views( $filter );
...
function rafflepress_pro_giveaway_get_data_total( $filter = null ) {
	global $wpdb;
...
function rafflepress_pro_giveaway_get_views( $filter = null ) {
	$views   = array();
...
function rafflepress_pro_duplicate_giveaway() {
	if ( check_ajax_referer( 'rafflepress_pro_duplicate_giveaway' ) ) {
		$id = '';
		if ( ! empty( $_GET['id'] ) ) {
			$id = absint( $_GET['id'] );
		}
		...
		if ( ! empty( $result ) ) {
			$r = $wpdb->insert(
				$tablename,
				array(
					'name'                => $result->name . ' ' . __( 'Copy', 'rafflepress-pro' ),
...
*/
function rafflepress_pro_archive_selected_giveaways() {
	if ( check_ajax_referer( 'rafflepress_pro_archive_selected_giveaways' ) ) {
...
*/
function rafflepress_pro_unarchive_selected_giveaways( $ids ) {
	if ( check_ajax_referer( 'rafflepress_pro_unarchive_selected_giveaways' ) ) {
		if ( current_user_can( apply_filters( 'rafflepress_list_users_capability', 'list_users' ) ) ) {
			if ( ! empty( $_GET['ids'] ) ) {
				$ids          = array_map( 'intval', explode( ',', $_GET['ids'] ) );
				$how_many     = count( $ids );
...
*/
function rafflepress_pro_delete_archived_giveaways() {
	if ( check_ajax_referer( 'rafflepress_pro_delete_archived_giveaways' ) ) {
		if ( current_user_can( apply_filters( 'rafflepress_list_users_capability', 'list_users' ) ) ) {
			global $wpdb;
			$tablename = $wpdb->prefix . 'rafflepress_giveaways';
...
function rafflepress_pro_start_giveaway() {
	if ( check_ajax_referer( 'rafflepress_pro_start_giveaway' ) ) {
		$id = '';
		if ( ! empty( $_GET['id'] ) ) {
			$id = absint( $_GET['id'] );
		}
...
function rafflepress_pro_end_giveaway() {
	if ( check_ajax_referer( 'rafflepress_pro_end_giveaway' ) ) {
		$id = '';
		if ( ! empty( $_GET['id'] ) ) {
			$id = absint( $_GET['id'] );
		}
...
function rafflepress_pro_enable_disable_giveaway() {
	if ( check_ajax_referer( 'rafflepress_pro_enable_disable_giveaway' ) ) {
		$id = '';
		if ( ! empty( $_GET['id'] ) ) {
			$id = absint( $_GET['id'] );
		}
...
function rafflepress_pro_save_slug() {
	if ( check_ajax_referer( 'rafflepress_pro_save_slug' ) ) {
		...
		if ( ! empty( $giveaway_slug ) ) {
			if ( empty( $this_slug_exist ) || $this_slug_exist != $giveaway_slug ) {
				...
				if ( ! empty( $slug_exist ) ) {
					$response = array(
						'status' => 'error',
						'msg'    => __( 'Sorry this url already exists. Please choose a new one.', 'rafflepress-pro' ),
					);
					wp_send_json( $response );
				}
...
function rafflepress_pro_save_giveaway() {
	if ( check_ajax_referer( 'rafflepress_pro_save_giveaway' ) ) {
		...
		array_walk_recursive( $settings, 'rafflepress_pro_convert_string_to_boolean' );
			...
			if ( ! empty( $giveaway_slug ) ) {
				if ( empty( $this_slug_exist ) || $this_slug_exist != $giveaway_slug ) {
					...
					if ( ! empty( $slug_exist ) ) {
						$response = array(
							'status' => 'error',
							'msg'    => __( 'Sorry the Page Permalink you assigned already exists. Please Choose a New Page Permalink under Settings > General', 'rafflepress-pro' ),
						);
...
function rafflepress_pro_get_utc_offset() {
	if ( check_ajax_referer( 'rafflepress_pro_get_utc_offset' ) ) {
		$_POST = stripslashes_deep( $_POST );
		...
		if ( ! empty( $starts_utc ) && time() < strtotime( $starts_utc . ' UTC' ) ) {
			$countdown_status = __( 'Starts in', 'rafflepress-pro' ) . ' ' . human_time_diff( time(), $starts_timestamp );
		} elseif ( ! empty( $ends_utc ) && time() > strtotime( $ends_utc . ' UTC' ) ) {
			$countdown_status = __( 'Ended', 'rafflepress-pro' ) . ' ' . human_time_diff( time(), $ends_timestamp ) . ' ago';
		}
...
/*
 * Save/Update Giveaways Publish
 */

function rafflepress_pro_save_publish() {
	if ( check_ajax_referer( 'rafflepress_pro_save_publish' ) ) {
		$_POST = stripslashes_deep( $_POST );
		...
		$active = 1;
		if ( empty( $_POST['giveaway_active'] ) || $_POST['giveaway_active'] == 'false' ) {
			$active = 0;
		}
...
function rafflepress_pro_save_template() {
	if ( check_ajax_referer( 'rafflepress_pro_save_template' ) ) {
		$_POST = stripslashes_deep( $_POST );
