...
if ( empty( $giveaway->active ) && empty( $_GET['rafflepress-preview'] ) ) {
	...
	//     $disabled_msg = __('Giveaway is Disabled', 'rafflepress-pro');
	// }
...
if ( $giveaway->starts != '0000-00-00 00:00:00' && $giveaway->ends != '0000-00-00 00:00:00' ) {
	if ( ! empty( $giveaway->starts ) && ! empty( $giveaway->ends ) ) {
		if ( time() < strtotime( $giveaway->starts . ' UTC' ) ) {
			...
			$msg               = __( 'This giveaway is not currently running.', 'rafflepress-pro' );
			...
			$countdown_running = false;
			$msg               = __( 'This giveaway has ended.', 'rafflepress-pro' );
		}
	}
}
...
<head>
	...
	if ( ! empty( $settings->page_background_color ) && empty( $_GET['iframe'] ) ) {
		echo "
...
#rafflepress-countdown,
	...
	<script data-cfasync="false"
		src="<?php echo RAFFLEPRESS_PRO_PLUGIN_URL; ?>public/js/iframeResizer.contentWindow.min.js?ver=<?php echo RAFFLEPRESS_PRO_VERSION; ?>">
	</script>
...
<body style="padding-top:0">
	<nav class="navbar navbar-default"
		...
		<div style="flex:1">
			<a href="<?php echo admin_url() . 'admin.php?page=rafflepress_pro#/'; ?>"><img style="    width: 45px;
	display: inline-block;
	margin-left:25px  " src="<?php echo RAFFLEPRESS_PRO_PLUGIN_URL; ?>/public/img/rafflepress-icon.png"
  ...
  align-items: center;">
			<div class="btn-group" role="group">
				<a href="<?php echo $actual_link; ?>&mode=live"
					class="btn btn-primary btn-preview <?php echo ( $preview_mode == 'live' ) ? 'active' : ''; ?>"><?php _e( 'Live Preview', 'rafflepress-pro' ); ?></a>
				<a href="<?php echo $actual_link; ?>&mode=actions"
					class="btn btn-primary btn-preview <?php echo ( $preview_mode == 'actions' ) ? 'active' : ''; ?>">
					<?php _e( 'Preview All Actions', 'rafflepress-pro' ); ?></a>
			</div>
		</div>
...
?>
		...
		<script>
		<?php $ajax_url = html_entity_decode( wp_nonce_url( admin_url( 'admin-ajax.php' ) . '?action=rafflepress_pro_giveaway_api', 'rafflepress_pro_giveaway_api' ) ); ?>
		var rafflepress_api_url = "<?php echo $ajax_url; ?>";

		<?php
		$fb_auth_integration_url = '';
		
		$fb_auth_integration_url = RAFFLEPRESS_PRO_WEB_API_URL . 'auth-fb-login';
		...
		var rafflepress_data =
			...
			echo json_encode(
				array(
					'api_url'                 => RAFFLEPRESS_PRO_CALLBACK_URL,
		...
		<script src="http://localhost:8081/app.js"></script>
			...
			?>
		<script
			src="<?php echo RAFFLEPRESS_PRO_PLUGIN_URL; ?>public/<?php echo $vue_app_folder; ?>/vue-frontend/js/app.js?ver=<?php echo RAFFLEPRESS_PRO_VERSION; ?>">
		</script>
		<script
			src="<?php echo RAFFLEPRESS_PRO_PLUGIN_URL; ?>public/<?php echo $vue_app_folder; ?>/vue-frontend/js/chunk-vendors.js?ver=<?php echo RAFFLEPRESS_PRO_VERSION; ?>">
		</script>
			<?php
